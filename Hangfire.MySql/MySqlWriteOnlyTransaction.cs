...
namespace Hangfire.MySql
{
    internal class MySqlWriteOnlyTransaction : JobStorageTransaction
    {
        ...
        public override void ExpireJob(string jobId, TimeSpan expireIn)
        {
            ...
            QueueCommand(x => 
                x.Execute(
                    $"update `{_storage.TablePrefix}Job` set ExpireAt = @expireAt where Id = @id",
        ...
        public override void PersistJob(string jobId)
        {
            ...
            QueueCommand(x => 
                x.Execute(
                    $"update `{_storage.TablePrefix}Job` set ExpireAt = NULL where Id = @id",
                    new { id = jobId }));
        }
        ...
        public override void SetJobState(string jobId, IState state)
        {
            ...
            QueueCommand(x => x.Execute(
                $"insert into `{_storage.TablePrefix}State` (JobId, Name, Reason, CreatedAt, Data) " +
                "values (@jobId, @name, @reason, @createdAt, @data); " +
                $"update `{_storage.TablePrefix}Job` set StateId = last_insert_id(), StateName = @name where Id = @id;",
                new
                {
                    jobId = jobId,
                    name = state.Name,
        ...
        public override void IncrementCounter(string key)
        {
            ...
            QueueCommand(x => 
                x.Execute(
                    $"insert into `{_storage.TablePrefix}Counter` (`Key`, `Value`) values (@key, @value)",
                    new { key, value = +1 }));
        ...
        public override void IncrementCounter(string key, TimeSpan expireIn)
        {
            ...
            QueueCommand(x => 
                x.Execute(
                    $"insert into `{_storage.TablePrefix}Counter` (`Key`, `Value`, `ExpireAt`) values (@key, @value, @expireAt)",
        ...
        public override void AddToSet(string key, string value, double score)
        {
            ...
            AcquireSetLock();
            QueueCommand(x => x.Execute(
                $"INSERT INTO `{_storage.TablePrefix}Set` (`Key`, `Value`, `Score`) " +
        ...
        public override void AddRangeToSet(string key, IList<string> items)
        {
            ...
            QueueCommand(x => 
                x.Execute(
                    $"insert into `{_storage.TablePrefix}Set` (`Key`, Value, Score) values (@key, @value, 0.0)", 
        ...
        public override void RemoveFromSet(string key, string value)
        {
            ...
            AcquireSetLock();
            QueueCommand(x => x.Execute(
                $"delete from `{_storage.TablePrefix}Set` where `Key` = @key and Value = @value",
                new { key, value }));
        }
        ...
        public override void ExpireSet(string key, TimeSpan expireIn)
        {
            ...
            QueueCommand(x => 
                x.Execute(
                    $"update `{_storage.TablePrefix}Set` set ExpireAt = @expireAt where `Key` = @key", 
        ...
        public override void InsertToList(string key, string value)
        {
            ...
            AcquireListLock();
            QueueCommand(x => x.Execute(
                $"insert into `{_storage.TablePrefix}List` (`Key`, Value) values (@key, @value)",
                new { key, value }));
        }
        ...
        public override void ExpireList(string key, TimeSpan expireIn)
        {
            ...
            QueueCommand(x => 
                x.Execute(
                    $"update `{_storage.TablePrefix}List` set ExpireAt = @expireAt where `Key` = @key", 
                    new { key = key, expireAt = DateTime.UtcNow.Add(expireIn) }));
        }
        ...
        public override void RemoveFromList(string key, string value)
        {
            ...
            AcquireListLock();
            QueueCommand(x => x.Execute(
                $"delete from `{_storage.TablePrefix}List` where `Key` = @key and Value = @value",
                new { key, value }));
        }
...
where lst.Key = @key
    and ranked.rank not between @start and @end",
        ...
        public override void PersistHash(string key)
        {
            ...
            QueueCommand(x => 
                x.Execute(
                    $"update `{_storage.TablePrefix}Hash` set ExpireAt = null where `Key` = @key", new { key = key }));
        }
        ...
        public override void PersistSet(string key)
        {
            ...
            QueueCommand(x => 
                x.Execute(
                    $"update `{_storage.TablePrefix}Set` set ExpireAt = null where `Key` = @key", new { key = key }));
        }
        ...
        public override void RemoveSet(string key)
        {
            ...
            QueueCommand(x => 
                x.Execute(
                    $"delete from `{_storage.TablePrefix}Set` where `Key` = @key", new { key = key }));
        }
        ...
        public override void PersistList(string key)
        {
            ...
            QueueCommand(x => 
                x.Execute(
                    $"update `{_storage.TablePrefix}List` set ExpireAt = null where `Key` = @key", new { key = key }));
        }
        ...
        public override void ExpireHash(string key, TimeSpan expireIn)
        {
            ...
            QueueCommand(x => 
                x.Execute(
                    $"update `{_storage.TablePrefix}Hash` set ExpireAt = @expireAt where `Key` = @key", 
                    new { key = key, expireAt = DateTime.UtcNow.Add(expireIn) }));
        }
