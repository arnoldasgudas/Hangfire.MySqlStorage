...
namespace Hangfire.MySql
{
    public class MySqlStorageConnection : JobStorageConnection
    {
        ...
        public override string CreateExpiredJob(Job job, IDictionary<string, string> parameters, DateTime createdAt, TimeSpan expireIn)
        {
            ...
            return _storage.UseConnection(connection =>
            {
                var jobId = connection.Query<int>(
                    $"insert into `{_storage.TablePrefix}Job` (InvocationData, Arguments, CreatedAt, ExpireAt) " +
                ...
                if (parameters.Count > 0)
                {
                    ...
                    connection.Execute(
                        $"insert into `{_storage.TablePrefix}JobParameter` (JobId, Name, Value) values (@jobId, @name, @value)", 
                        parameterArray);
                }
        ...
        public override void SetJobParameter(string id, string name, string value)
        {
            ...
            {
                connection.Execute(
                    $"insert into `{_storage.TablePrefix}JobParameter` (JobId, Name, Value) " +
        ...
        public override string GetJobParameter(string id, string name)
        {
            ...
            return _storage.UseConnection(connection => 
                connection.Query<string>(
                    "select Value " +
                    $"from `{_storage.TablePrefix}JobParameter` " +
            ...
            return _storage.UseConnection(connection =>
            {
                var sqlState = 
                    connection.Query<SqlState>(
                        ...
                        $"from `{_storage.TablePrefix}State` s inner join `{_storage.TablePrefix}Job` j on j.StateId = s.Id " +
                        "where j.Id = @jobId", 
        ...
        public override void AnnounceServer(string serverId, ServerContext context)
        {
            ...
            {
                connection.Execute(
                    $"INSERT INTO `{_storage.TablePrefix}Server` (Id, Data, LastHeartbeat) " +
        ...
        public override void RemoveServer(string serverId)
        {
            ...
            {
                connection.Execute(
                    $"delete from `{_storage.TablePrefix}Server` where Id = @id",
                    new { id = serverId });
            });
        }
        ...
        public override void Heartbeat(string serverId)
        {
            ...
            {
                connection.Execute(
                    $"update `{_storage.TablePrefix}Server` set LastHeartbeat = @now where Id = @id",
        ...
        public override int RemoveTimedOutServers(TimeSpan timeOut)
        {
            ...
            return
                _storage.UseConnection(connection =>
                    connection.Execute(
                        $"delete from `{_storage.TablePrefix}Server` where LastHeartbeat < @timeOutAt",
            ...
            return
                _storage.UseConnection(connection =>
                    connection.Query<int>(
                        $"select count(`Key`) from `{_storage.TablePrefix}Set` where `Key` = @key",
                        new {key = key}).First());
        }
        ...
        public override List<string> GetRangeFromSet(string key, int startingFrom, int endingAt)
        {
            ...
            return _storage.UseConnection(connection =>
                connection
                    .Query<string>($@"
select `Value` 
from `{_storage.TablePrefix}Set` s 
    inner join (
	    select tmp.Id, @rownum := @rownum + 1 AS rank
	    from `{_storage.TablePrefix}Set` tmp,
...
where s.`Key` = @key 
    and  ranked.rank between @startingFrom and @endingAt",
            ...
            return
                _storage.UseConnection(connection =>
                {
                    var result = connection.Query<string>(
                        $"select Value from `{_storage.TablePrefix}Set` where `Key` = @key",
                        new {key});
        ...
        public override long GetCounter(string key)
        {
            ...
            string query = $@"
select sum(s.`Value`) from (select sum(`Value`) as `Value` from `{_storage.TablePrefix}Counter`
where `Key` = @key
union all
select `Value` from `{_storage.TablePrefix}AggregatedCounter`
where `Key` = @key) as s";
                ...
                _storage
                    .UseConnection(connection => 
                        connection.Query<long>(
                            $"select count(Id) from `{_storage.TablePrefix}Hash` where `Key` = @key", 
                            new { key = key }).Single());
        }
            ...
            return _storage.UseConnection(connection =>
            {
                var result = 
                    connection.Query<DateTime?>(
                        $"select min(ExpireAt) from `{_storage.TablePrefix}Hash` where `Key` = @key", 
                        new { key = key }).Single();
        ...
        public override long GetListCount(string key)
        {
            ...
            return 
                _storage
                    .UseConnection(connection => 
                        connection.Query<long>(
                            $"select count(Id) from `{_storage.TablePrefix}List` where `Key` = @key", 
                            new { key = key }).Single());
        }
        ...
        public override TimeSpan GetListTtl(string key)
        {
            ...
            return _storage.UseConnection(connection =>
            {
                var result = 
                    connection.Query<DateTime?>(
                        $"select min(ExpireAt) from `{_storage.TablePrefix}List` where `Key` = @key", 
                        new { key = key }).Single();
                if (!result.HasValue) return TimeSpan.FromSeconds(-1);
        ...
        public override string GetValueFromHash(string key, string name)
        {
            ...
            return 
                _storage
                    .UseConnection(connection => 
                        connection.Query<string>(
                            $"select `Value` from `{_storage.TablePrefix}Hash` where `Key` = @key and `Field` = @field", 
        ...
        public override List<string> GetRangeFromList(string key, int startingFrom, int endingAt)
        {
            ...
            string query = $@"
select `Value` 
from `{_storage.TablePrefix}List` lst
    inner join (
        select tmp.Id, @rownum := @rownum + 1 AS rank
	    from `{_storage.TablePrefix}List` tmp,
            (select @rownum := -1) r 
...
order by lst.Id desc";
        ...
        public override List<string> GetAllItemsFromList(string key)
        {
            ...
            string query = $@"
select `Value` from `{_storage.TablePrefix}List`
where `Key` = @key
order by Id desc";
        ...
        public override TimeSpan GetSetTtl(string key)
        {
            ...
            return _storage.UseConnection(connection =>
            {
                var result = 
                    connection
                        .Query<DateTime?>(
                            $"select min(ExpireAt) from `{_storage.TablePrefix}Set` where `Key` = @key", 
                            new { key = key }).Single();
                if (!result.HasValue) return TimeSpan.FromSeconds(-1);
        ...
        public override void SetRangeInHash(string key, IEnumerable<KeyValuePair<string, string>> keyValuePairs)
        {
            ...
            _storage.UseTransaction(connection =>
            {
                ...
                {
                    connection.Execute(
                        $"insert into `{_storage.TablePrefix}Hash` (`Key`, Field, Value) " +
        ...
        public override Dictionary<string, string> GetAllEntriesFromHash(string key)
        {
            ...
            return _storage.UseConnection(connection =>
            {
                var result = connection.Query<SqlHash>(
                    $"select Field, Value from `{_storage.TablePrefix}Hash` where `Key` = @key",
                    new {key})
